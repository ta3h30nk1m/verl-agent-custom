...
usage() {
    echo "Usage:"
    echo "  run_hyperclovax_lora_kd_single_gpu.sh [save_path] [other_configs...]"
    echo "  run_hyperclovax_lora_kd_single_gpu.sh <nproc_per_node> <save_path> [other_configs...]"
}
...
if [ "$#" -gt 0 ] && [[ "$1" =~ ^[0-9]+$ ]]; then
    ...
    nproc_per_node=${NPROC_PER_NODE:-1}
    save_path=${SAVE_PATH:-outputs/mind2web_hyperclovax_lora_kd}
fi
...
KD_LOSS_TYPE=${KD_LOSS_TYPE:-rkl}
KD_TEMPERATURE=${KD_TEMPERATURE:-1.0}
KD_JSD_BETA=${KD_JSD_BETA:-0.5}
KD_SKEW_ALPHA=${KD_SKEW_ALPHA:-0.1}
KD_COEFF=${KD_COEFF:-0.0}
KD_BALANCING_COEFF=${KD_BALANCING_COEFF:-0.5}

if [ "${SP_SIZE}" != "1" ]; then
    echo "KD trainer currently supports SP_SIZE=1 only."
    exit 1
fi

if [ -z "${SAVE_PATH_SUFFIX+x}" ]; then
    SAVE_PATH_SUFFIX="ep${TOTAL_EPOCHS}_lr${LR}_r${LORA_RANK}_kd${KD_LOSS_TYPE}_t${KD_TEMPERATURE}_bal${KD_BALANCING_COEFF}"
fi
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-mind2web-hyperclovax-lora-kd-${KD_LOSS_TYPE}-r${LORA_RANK}}
LOGGER=${LOGGER:-"['console','wandb']"}

torchrun --standalone --nnodes=1 --nproc_per_node="${nproc_per_node}" \
    -m verl.trainer.fsdp_sft_kd_trainer \
    ...
    distillation.loss_type="${KD_LOSS_TYPE}" \
    distillation.temperature="${KD_TEMPERATURE}" \
    distillation.jsd_beta="${KD_JSD_BETA}" \
    distillation.skew_alpha="${KD_SKEW_ALPHA}" \
    distillation.kd_coeff="${KD_COEFF}" \
    distillation.kd_balancing_coeff="${KD_BALANCING_COEFF}" \
    optim.lr="${LR}" \
