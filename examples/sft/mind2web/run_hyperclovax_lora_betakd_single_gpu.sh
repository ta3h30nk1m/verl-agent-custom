...
usage() {
    echo "Usage:"
    echo "  run_hyperclovax_lora_betakd_single_gpu.sh [save_path] [other_configs...]"
    echo "  run_hyperclovax_lora_betakd_single_gpu.sh <nproc_per_node> <save_path> [other_configs...]"
}
...
if [ "$#" -gt 0 ] && [[ "$1" =~ ^[0-9]+$ ]]; then
    ...
    nproc_per_node=${NPROC_PER_NODE:-1}
    save_path=${SAVE_PATH:-outputs/mind2web_hyperclovax_lora_betakd}
fi
...
TEACHER_LORA_ADAPTER_PATH=${TEACHER_LORA_ADAPTER_PATH:-null}
...
BETAKD_LOSSES=${BETAKD_LOSSES:-fkl}
BETAKD_TEMPERATURE=${BETAKD_TEMPERATURE:-1.0}
BETAKD_WEIGHTING=${BETAKD_WEIGHTING:-instance}
BETAKD_WEIGHT_CE=${BETAKD_WEIGHT_CE:-false}
BETAKD_HIDDEN_DIM=${BETAKD_HIDDEN_DIM:-16}

if [ "${SP_SIZE}" != "1" ]; then
    echo "BetaKD trainer currently supports SP_SIZE=1 only."
    exit 1
fi

if [ -z "${SAVE_PATH_SUFFIX+x}" ]; then
    SAVE_PATH_SUFFIX="ep${TOTAL_EPOCHS}_lr${LR}_r${LORA_RANK}_betakd_${BETAKD_LOSSES}_${BETAKD_WEIGHTING}"
fi
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-mind2web-hyperclovax-lora-betakd-${BETAKD_LOSSES}-${BETAKD_WEIGHTING}-r${LORA_RANK}}
LOGGER=${LOGGER:-"['console','wandb']"}

torchrun --standalone --nnodes=1 --nproc_per_node="${nproc_per_node}" \
    -m verl.trainer.fsdp_sft_betakd_trainer \
    ...
    teacher.enable=true \
    teacher.model_path="${TEACHER_MODEL_PATH}" \
    teacher.lora_adapter_path="${TEACHER_LORA_ADAPTER_PATH}" \
    teacher.trust_remote_code=true \
    teacher.torch_dtype="${TEACHER_TORCH_DTYPE}" \
    teacher.attn_implementation="${TEACHER_ATTN_IMPLEMENTATION}" \
    beta_kd.losses="${BETAKD_LOSSES}" \
    beta_kd.temperature="${BETAKD_TEMPERATURE}" \
    beta_kd.weighting="${BETAKD_WEIGHTING}" \
    beta_kd.weight_ce="${BETAKD_WEIGHT_CE}" \
    beta_kd.hidden_dim="${BETAKD_HIDDEN_DIM}" \
    optim.lr="${LR}" \
